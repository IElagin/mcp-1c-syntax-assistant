...
472 теста. Настройки pytest — в `pytest.ini` в корне репозитория, общие
...
|---|---|---|
| `unit` | Быстрые тесты на заглушках, без внешних зависимостей | 280 |
...
| `real_index` | Требует заполненный боевой индекс `help1c_docs` | 46 |
...
Сумма по `unit` и `integration` (280 + 58 = 338) меньше 472: часть тестов не
...
41 тест с маркером `elasticsearch` требует живого кластера. Из них 32 несут
...
бы хуже красного. Оставшиеся 9 строят собственный изолированный индекс и
