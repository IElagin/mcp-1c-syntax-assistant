...
одного языка стёрла бы уже проиндексированные карточки.
...
Книга, которую не удалось прочитать или которая при разборе потеряла больше
5% страниц (`MAX_TOLERATED_PAGE_LOSS_SHARE` в `src/core/constants.py`),
переиндексацию отклоняет — старый индекс остаётся как был, вместо того чтобы
стать наполовину пустым.
