...
- **Параметры**: query (строка поиска), kind (тип элемента), object (объект), limit (количество кандидатов, по умолчанию 10, не более 200)
- **Пример**: "СтрДлина", "ТаблицаЗначений.Добавить"

### 2. get_1c_element
Полная карточка элемента: строка вызова, варианты вызова, параметры с типами
и обязательностью, тип возврата, доступность, версия, описание, пример
- **Параметры**: name (точное имя), object (объект, обязателен при неуникальном имени), variant (вариант вызова)
- **Пример**: "СтрДлина", "ВРег"

### 3. list_1c_object_members
Состав объекта 1С: методы, свойства, события, конструкторы
- **Параметры**: object (имя объекта), members (какие элементы перечислить), limit
