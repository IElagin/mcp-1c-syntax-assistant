...
def canonical_source_file(source_file: Optional[str]) -> str:
    """Внутрикнижный путь страницы в единой записи — через прямой слэш.

    source_file приходит из вывода `7z l` и потому зависит от того, где
    собирался индекс: 7-Zip под Windows печатает «objects\\Global context.html»,
    p7zip под Linux — «objects/Global context.html». Само по себе это неважно,
    пока путь не становится ключом: достройка английских имён склеивает русский
    и английский индексы ровно по source_file (см. src/parsers/name_backfill.py).

    Ровно так и сломалось на живом стенде: русский индекс собран на хосте
    (23 125 путей со слэшем «\\»), английский — в контейнере (23 104 пути с
    «/»), term-склейка давала 0 совпадений из 23 104 возможных, и 653 документа
    остались без английского имени, хотя парная английская страница была у
    каждого. Отказ был молчаливым: достройка возвращала 0 и выглядела
    работающей — «кандидатов нет» и «ключ не сходится» с её стороны неотличимы.

    Прямой слэш выбран каноном, потому что так путь и записан внутри самих
    книг; обратный — артефакт одной из платформ сборки.
    """
