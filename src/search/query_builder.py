...
class QueryBuilder:
    ...
    def parse_qualified_name(query: str):
        ...
        Правило синтаксическое и намеренно узкое: одна точка и ни одного
        пробела. Оно не покрывает 28% точечных путей справки — имена объектов
        там сплошь и рядом с пробелами («Расширение формы клиентского
        приложения для плана видов характеристик.РежимВыбора») и с точками
        внутри («WSСсылкаМенеджер.<Имя WS-Ссылки>.ПолучитьWSОпределения»), —
        и расширить его одним синтаксисом нельзя: «Как добавить строку. Пример
        кода» устроен ровно так же и обращением к элементу не является.
        Поэтому широкий разбор идёт через SearchService._qualified_split, где
        имя объекта проверяется по индексу, а эта функция остаётся запасным
        вариантом для случая, когда объект в справке не найден: она сохраняет
        фильтр по объекту, и пустая выдача честно говорит, что виноват объект,
        а не отсутствие элемента.
        """
        text = (query or "").strip()
    ...
    @staticmethod
    def split_points(query: str):
        """Все разрезы 'A.B' — от самого длинного имени объекта к самому короткому.

        Порядок не косметический: у «WSСсылкаМенеджер.<Имя WS-Ссылки>.Получить
        WSОпределения» объектом справки является «WSСсылкаМенеджер.<Имя
        WS-Ссылки>» (у 220 объектов имя само содержит точки), и разрез по
        первой точке дал бы несуществующий объект «WSСсылкаМенеджер» с
        «элементом» «<Имя WS-Ссылки>.ПолучитьWSОпределения».
        """
        text = (query or "").strip()
        points = []
